...
| 内容 | 本地来源证据 | 处理方式 |
| --- | --- | --- |
| `prompt-translator-agent` 与 `agents/prompt-translator-agent/` | 本仓库新增的需求翻译 Skill 与 Agent，维护者 walterkken | 新增内容按 `skills/prompt-translator-agent/LICENSE` 的 MIT 条款使用；不改变其他组件许可 |
| `skill-adapter` 与 `agents/skill-adapter/` | 本仓库新增的 Skill 适配工具，维护者 walterkken | 新增内容按 `skills/skill-adapter/LICENSE` 的 MIT 条款使用；官方文档仅链接引用，无官方背书 |
